...
- Reutilización de Sesiones
	- Los parámetros quedan guardados por un tiempo en el cache SSL
	- El `ClientHello` contiene un `SessionID` que identifica una sesión anterior.
	- En el `ServerHello`se puede aceptar o rechazar incluyendo otro `SessionID`.
### Funcionamiento
```mermaid
sequenceDiagram
...
```mermaid
sequenceDiagram
	Note over Cliente,Servidor: Key Exchange
	Cliente->>Servidor: ClientHello
	Servidor->>Cliente: ServerHello
	
	Note over Cliente,Servidor: Server Params
	Servidor->>Cliente: EncryptedExtensions
	Servidor->>Cliente: CertificateRequest (*)
	
	Note over Cliente,Servidor: Auth Servidor
	Servidor->>Cliente: Certificate (*)
	Servidor->>Cliente: CertificateVerify (*)
	Servidor->>Cliente: Finish
	Servidor->>Cliente: ApplicationData
	
	Note over Cliente,Servidor: Auth Cliente
	Cliente->>Servidor: Certificate (*)
	Cliente->>Servidor: CertificateVerify (*)
	Cliente->>Servidor: Finish
	Cliente->>Servidor: ApplicationData
```
---