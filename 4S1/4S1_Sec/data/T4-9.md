...
- **Def**. Inyecta script malicioso en datos que recibe un cliente web
- **Tipos**
	- **Persistentes (almacenados)**
		- Script almacenado en servidor malicioso que consulta la víctima.
		- Todos los visitantes del servidor se verán afectados.
	- **No persistentes (reflejados)**
		- Vulnerabilidad del sitio web permite q atacante incruste el script in la URL
		- Link malicioso dirigido solo al usuario objetivo.
	- **Basados en DOM**
		- Cuando se monta el árbol DOM se ejecuta un script con dos elementos:
			- Fuente (source) que permite obtener el código malicioso (lee datos).
			- Sumidero (sink) que permite ejecutarlo (escribe datos)
- **Defensas**
	- Código web sin vulnerabilidades
	- WAF (Web Application Firewall). Monitoriza y bloquea tráfico HTTP.