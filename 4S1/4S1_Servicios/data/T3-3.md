...
### Interfaz Usuario (CMD)

| Comando         | Significado                            |
| --------------- | -------------------------------------- |
| `open sitio`    | Establece conexión con el sitio        |
| `dir`           | Solicita listado del directorio remoto |
| `cd directorio` | Cambia el directorio remoto            |
| `mkdir d`       | Crear directorio `d` remoto            |
| `ascii/binary`  | Cambiar tipo de transferencia          |
| `passive`       | Cambiar servidor a modo pasivo         |
| `get fichero`   | Descarga del fichero                   |
| `mget *.txt`    | Descarga de varios, según patrón       |
| `put fichero`   | Subida del fichero                     |
| `mput *.py D`   | Subida de varios, según patrón         |
| `rename`        | Cambia nombre a fichero remoto         |
| `exit`          | Finalizar conexión                     |

---
## Secure Copy (SCP)
- Permite subida y bajada de archivos entre un cliente y un servidor SSH.
	- Subida: `scp <Archivo_Local> <Usuario_R>@<IP_R>:<Path_R>/<Archivo_R>`
	- Bajada: `scp <Usuario_R>@<IP_R>:<Path_R>/<Archivo_R> <Archivo_Local>`
- No permite crear directorios ni listarlos. Para eso usar SSH.
- Conexión binaria cifrada y opcionalmente comprimida.
---
## Secure FTP
- Nuevo protocolo que funciona sobre un túnel SSH.
- Funcionalidad y uso similar a FTP pero cifrado.
---