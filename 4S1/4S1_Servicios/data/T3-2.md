...
- Las claves privado/públicas propias normalmente se almacenan en `~/.ssh/`
### FASE 1. Establecimiento de conexión (`SSH-TRANS`)
...
### FASE 2. Autenticación de cliente (`SSH-USERAUTH`)
#### (Opción 1) *Keyboard Interactive* o Contraseña
- **Def**. El cliente pide una contraseña al usuario, la envía al servidor y este la verifica.
- **Ventaja**: No requiere setup inicial
- **Desventaja** Pide contraseña en cada sesión. No permite trabajo desatendido.
#### (Opción 2) Clave pública
-  **Proceso**
	1. Cliente envía su clave pública
	2. Si servidor la reconoce envía un "reto" (número de `256b`) cifrado con esta
	3. Cliente descifra reto, hace una serie de operaciones prefijadas y envía resultado.
	4. Servidor hace las mismas operaciones y, si los resultados son iguales, autentica.
-  **Ventajas**: No se transmiten secretos y configurable al lado del servidor.
- **Desventajas**:
	- Clave privada en cliente está protegida por contraseña.
- **Solución**: Agentes SSH
	1. Proceso que mantiene copia de claves privadas en memoria RAM.
	2. La contraseña solo se pide la primera vez durante la carga de la clave.
	3. Agente es encargado de resolver retos a través de un socket localhost seguro.
### FASE 3. Creación de Canales (`SSH-CONNECT`)
- **Proceso**
	1. Se escoge un número (libre) para el canal.
	2. Cualquiera de los extremos solicita la creación del canal incluyendo el tipo y el número.
	3. El otro extremo acepta o rechaza la creación del canal.
	4. Paquetes en la misma conexión TCP cifrada son multiplexados por el número de canal.
- **Utilidad Canales**
	- Sesión interactiva remota (similar a telnet)
	- Ejecución de comandos remotos no interactivos
	- Transferencia de ficheros
	- Redirección de puertos (túneles SSH cifrados)
- Explicación Túneles Cifrados SSH
	- **Def**.
		- Permite a un cliente origen (`L`) 
		- Conectarse a una máquina destino (`S`)
		- Usando un servidor SSH intermedio (`R`)
		- De forma que la conexión entre `L` y `R` es cifrada con SSH.
	- **Creación**: `ssh -L[IP_L:]puerto_L:IP_R:puerto_R [-p puerto_S] user@IP_S`
		- Por defecto `IP_L` es `INADDR_ANY` (cualquiera puede conectarse al túnel)
---